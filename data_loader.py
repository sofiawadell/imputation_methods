...
def data_loader_norm_factor_mice_imputed_data(data_name, miss_rate):
    ...
    if data_name in datasets.keys():   
        train_imp_factor_norm_data_x = pd.read_csv('imputed_data/norm_factor_imputed_mice_train_data/norm_factor_imputed_mice_'+data_name+'_train_'+missingness_str+'.csv')
    else:
        ValueError("Dataset not found")

    # Load normalized MICE imputed test missing data 
    if data_name in datasets.keys():   
        test_imp_factor_norm_data_x = pd.read_csv('imputed_data/norm_factor_imputed_mice_test_data/norm_factor_imputed_mice_'+data_name+'_test_'+missingness_str+'.csv') 
    else:
        ValueError("Dataset not found")
